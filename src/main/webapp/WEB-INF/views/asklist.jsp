...
<script>
// 판매 입찰 취소
function askDelete (number) {
	var check = confirm('입찰을 취소 하시겠습니까?');
	if (check) {
		console.log(number);
		location.href = 'askdelete?asknumber=' + number;
		alert('최소 되었습니다.');
	} else {
		alert('삭제를 취소 합니다.');
	}
		...
		<!-- 페이징 -->
			<table>
				...
				<c:forEach var="ask" items="${askList}">
					<tr>
						<td><a href="productview?pnumber=${ask.askpnumber}&page=${paging.page}">${ask.askpnumber}</a></td>
						<td>${ask.askprice}</td>
						<td>${ask.asksize}</td>
						<td><a href="askupdate?asknumber=${ask.asknumber}">수정</a></td>
						<td><button onclick="askDelete('${ask.asknumber}')">취소</button></td>
					</tr>
				</c:forEach>
			</table>
				...
				<c:otherwise>
					<a href="asklistpage?page=${paging.page-1}">[이전]</a>&nbsp;
				</c:otherwise>
			</c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
					<a href="asklistpage?page=${paging.page+1}">[다음]</a>
				</c:otherwise>
			</c:choose>
