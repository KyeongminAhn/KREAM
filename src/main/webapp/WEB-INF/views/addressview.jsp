...
<!-- 로그 아웃 -->
	function addressDelete() {
		var check = confirm('주소를 삭제 하시겠습니까?')
		if (check) {
			location.href = "addressdelete?id=${address.id}"
			alert('주소 삭제 성공, 마이페이지로 이동합니다.')
		}else {
			alert('주소 삭제 취소')
		}
	}
...
<body>
	...
	<div id="content">
		<h6>content</h6>
		우편 번호 : ${address.postcode} <br>
		도로명 주소 : ${address.road} <br>
		지번 주소 : ${address.jibun} <br>
		상세 주소 : ${address.detail} <br>
		참고 항목 : ${address.extra} <br>
		<br>
		<button onclick="addressDelete()">주소 삭제</button>
